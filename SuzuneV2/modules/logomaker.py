...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./SuzuneV2/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./SuzuneV2/resources/Chopsic.otf", 330)
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="💕 Made By @Suzune_Support Support @Suzune_Support ")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @Suzune_Support, {e}')
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./SuzuneV2/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    ...
    fname2 = "LogoByInnexia.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By Suzune Team")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @Suzune_Support {e}')
